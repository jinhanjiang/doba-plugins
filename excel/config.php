...
class ExcelPlugin extends BasePlugin {
    ...
    public function read($params = array())
    {
        ...
        $cacheSettings = array('memoryCacheSize'=>'16MB', 'cacheTime' => 300);
        PHPExcel_Settings::setCacheStorageMethod($cacheMethod,$cacheSettings);

        $iputFileType = PHPExcel_IOFactory::identify($filepath);
        $objReader = PHPExcel_IOFactory::createReader($iputFileType);
    ...
    public function write($params = array())
    {
        ...
        $colors = array(
            'black'=>PHPExcel_Style_Color::COLOR_BLACK, 
            'white'=>PHPExcel_Style_Color::COLOR_WHITE, 
            'red'=>PHPExcel_Style_Color::COLOR_RED, 
            'darkred'=>PHPExcel_Style_Color::COLOR_DARKRED, 
            'blue'=>PHPExcel_Style_Color::COLOR_BLUE, 
            'darkblue'=>PHPExcel_Style_Color::COLOR_DARKBLUE, 
            'green'=>PHPExcel_Style_Color::COLOR_GREEN, 
            'darkgreen'=>PHPExcel_Style_Color::COLOR_DARKGREEN, 
            'yellow'=>PHPExcel_Style_Color::COLOR_YELLOW, 
            'darkyellow'=>PHPExcel_Style_Color::COLOR_DARKYELLOW
        );
        ...
        for($i = 0; $i < $dataCnt; $i ++ )
        {
            ...
            for($j = 0; $j < $rowCnt; $j ++)
            {
                ...
                if($bg) 
                {
                    ...
                    $objActSheet->getStyle($col_tag)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
                ...
                $objActSheet->setCellValueExplicit($col_tag, $value, PHPExcel_Cell_DataType::TYPE_STRING);
            }
        }
        ...
        //$objWriter = new PHPExcel_Writer_Excel2007($objPHPExcel); //用于2007格式
        //$objWriter->setOffice2003Compatibility(true);

        $suffix = '.xlsx'; $exportType = 'Excel2007';
        if(in_array($options['suffix'], array('xls', 'xlsx'))) {
            $suffix = '.'.$options['suffix'];
            if('xls' == $options['suffix']) $exportType = 'Excel5'; 
            else{
                $suffix = '.xlsx';  $exportType = 'Excel2007';
            } 
        }
        ...
        $filename = $this->genFilename($suffix);
    ...
    public function getColRow($file)
    {
        ...
        $iputFileType = PHPExcel_IOFactory::identify($file);
        $objReader = PHPExcel_IOFactory::createReader($iputFileType);
